...
const users: Record<string, User> = retrieveUsers();
const transactions: Transaction[] = retrieveTransactions();
...
function retrieveUsers(): Record<string, User> {
...
function retrieveTransactions(): Transaction[] {
...
function createAccount(username: string): CreationOutcome {
...
function processPayment(
	...
	const inputTokenHash = scryptSync(payerToken, payerSalt, 64).toString('hex');
...
api.post('/new-bank-account', (request, response) => {
	const outcome = createAccount(request.body?.username);
...
api.post('/send-money', (request, response) => {
    ...
    if (typeof encodedAuth !== 'string') response.status(400).json({
        status: 400,
        error: {
            code: 'INVALID_AUTH',
...
api.get('/transaction-info', (request, response) => {
	...
	const isOwnTransaction = users[user].transaction_ids.includes(request.body.transaction_id);
	const transaction = transactions.find((transaction) => transaction.id == request.body.transaction_id);
